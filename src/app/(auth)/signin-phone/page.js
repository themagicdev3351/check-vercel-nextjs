...
import { generateOtpMobile, verifyMobileOtp } from "@/features/auth/authSlice";
...
const SignInPhone = () => {
    ...
    const handleVerifyOtp = async (data) => {
        setLoading(true);
        try {
            const { number, countryCode, otp } = data;
            const payload = {
                number: data.number,
                countryCode: data.countryCode,
                otp,
            ...
            const verifyResponse = await dispatch(verifyMobileOtp(payload));
            if (verifyResponse?.payload?.success) {
                ...
                toast({
                    title: "Login Failed",
                    description: verifyResponse?.payload?.message || "An unknown error occurred.",
                    status: "error",
                });
            }
        } catch (error) {
